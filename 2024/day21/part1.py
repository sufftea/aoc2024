...
class State:
    ...
    def apply_action(self, action: str) -> "State | None":
        ...
        if action in MOVE_ACTIONS:
            next_arm_c = _move(self.arm_c, ACTION_TO_OFFSET[action])

            if next_arm_c not in ROBOT_BUTTONS:
                return None

            return replace(result, arm_c=next_arm_c)
        if action == "X":
            return None

        action = ROBOT_BUTTONS[self.arm_c]
        if action in MOVE_ACTIONS:
            next_arm_b = _move(self.arm_b, ACTION_TO_OFFSET[action])

            if next_arm_b not in ROBOT_BUTTONS:
                return None

            return replace(result, arm_b=next_arm_b)
        if action == "X":
            return None

        action = ROBOT_BUTTONS[self.arm_b]
        if action in MOVE_ACTIONS:
            next_arm_a = _move(self.arm_a, ACTION_TO_OFFSET[action])

            if next_arm_a not in NUMPAD_BUTTONS:
                return None

            return replace(result, arm_a=next_arm_a)
        if action == "X":
            return None

        numpad_code = NUMPAD_BUTTONS[self.arm_a]
